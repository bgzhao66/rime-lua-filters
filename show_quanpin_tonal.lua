...
local function filter_func(input, env)
  ...
  for cand in input:iter() do
    ...
    local py_len = math.min(3*N, total_len)
    local py_code = active_input:sub(1, py_len)
    local cj_code = active_input:sub(3*N+1, total_len)

    local sanpin = bopomofo.sanpin_to_quanpin_full(py_code)
    cand.preedit = string.format("%s%s", sanpin, cj_code)
